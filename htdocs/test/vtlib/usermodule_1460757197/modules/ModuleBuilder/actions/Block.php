...
class ModuleBuilder_Block_Action extends Settings_Vtiger_Index_Action 
{
   ...
   public function updateSequenceNumber(Vtiger_Request $request) 
   {
        ...
        try
		{
			...
			{
				$mblocks 	= $session['tks_module_builder']['tks_blocks'][$value - 1];
				...
				if($key != $value)
				{	
					unset($session['tks_module_builder']['tks_blocks'][$value - 1]); 
					$session['tks_module_builder']['tks_blocks'] = array_values($session['tks_module_builder']['tks_blocks']);
					array_splice( $session['tks_module_builder']['tks_blocks'], $key-1, 0, array($mblocks) );
				}	
			}	
    ...
    public function delete(Vtiger_Request $request)
	{
		...
		$fieldarr = $_SESSION[ 'tks_module_builder' ][$token][ 'tks_blocks' ][ $blockId ][ 'tks_fields' ];
