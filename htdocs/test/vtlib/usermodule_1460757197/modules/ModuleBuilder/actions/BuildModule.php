...
class ModuleBuilder_BuildModule_Action extends Settings_Vtiger_Index_Action 
{
    ...
    public function saveModule( Vtiger_Request $request ) 
	{
		...
		$this -> modulename		 = ucfirst( strtolower( $request -> get( 'tks_modulename' ) ) );
		$this -> dst			 = $this -> dst.$this -> modulename;
		$this -> tks_entity		 = $request->get('tks_entity');
	...
	private function response( $result ) 
	{
		...
		try
		{
			...
			if( empty( $result ) )
			{
				...
				$response->setResult( array( 'success'=>true, 'saved' => 'success'));
			}
			else
			{
	...
	private function create_tablesql( $tablename, $type)
	{
		...
		$primarykey	= '`'.strtolower( $this -> modulename).'id`'.' int(11) default NULL, ';
	...
	private function export( $request) 
	{
		...
		$label = ucfirst( $request->get( 'tks_modulelabel'));
		...
		$parentText = $xml->createTextNode( $request->get( 'tks_parent'));
		...
		$dependencies   = $xml->createElement( "dependencies");			
			...
			$vtiger_versionText = $xml->createTextNode( '6.4.0');
		...
		$licence   			= $xml->createElement( "license");			
			...
			$lic 			= str_replace( '##moduleLable##', $request ->get ( 'tks_modulelabel'), $lic);
		 ...
		 $customviews->appendChild( $customview);
			...
			if ( $res === TRUE)
			{
				...
				else
				{
				  ...
				  $lang_replaced 		= str_replace( '##moduleLable##', $request ->get ( 'tks_modulelabel'), $lang_replaced);
