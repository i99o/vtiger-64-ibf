    ...
    <div class="listViewActionsDiv row-fluid">
        <span class="btn-toolbar span4">
            ...
            {if $EDIT eq 'permitted'}
                <span class="btn-group"><button class="btn addButton" type="submit" onclick="this.form.view.value = 'Edit';"><i class="icon-plus icon-white"></i>&nbsp;<strong>{vtranslate("LBL_ADD_TEMPLATE","PDFMaker")}</strong></button></span>
            {/if}
            {if $IS_ADMIN eq '1' && $TO_UPDATE eq 'true'}
                <span class="btn-group"><button class="btn lastImport" onclick="window.location.href = 'index.php?module=PDFMaker&view=update'; return false;">{vtranslate("LBL_UPDATE","PDFMaker")}</button></span>
            {/if}
                    ...
                    {if $IS_ADMIN eq "1"}
                        <a href='index.php?module=PDFMaker&view=License'>
                            <span class="btn btn-danger">
                                {vtranslate("LBL_ACTIVATE_BTN",$MODULE)}
                            </span>
                        </a>
                    {else}
                                ...
                                {if $FOLDERS neq ''}
                                        <optgroup id="foldersBlock" label='{vtranslate('LBL_FOLDERS', $MODULE)}' >
                                                {foreach item=FOLDER from=$FOLDERS}
                                                        <option data-foldername="{$FOLDER->getName()}" data-id="{$DEFAULT_CUSTOM_FILTER_ID}">{$FOLDER->getName()}</option>
                                                {/foreach}
                                        </optgroup>
                                {/if}
                        </select>
